...
This document provides instructions on how to deploy the example implementation of the different priority route pattern as part of [Patterns for using floating IP addresses in Compute Engine](https://cloud.google.com/architecture/patterns-for-floating-ip-addresses-in-compute-engine#using_different_priority_routes) using [Terraform](https://www.terraform.io/).
...
* Two static routes with different priorities using the floating IP address as destination and one of the nginx instances as next hop
...
1. In the code that you downloaded, enter the `5-routes-priority` subdirectory: `cd floating-ip-patterns/5-routes-priority`.
...
You have now deployed the example implementation for the different priority routes pattern.
...
1. On the `nginx-primary` VM, stop the `nginx` service by running:
   ...
   ```
1. Run the `curl` command from above repeatedly on the `client` instance and after a while all requests should return `This is server 2`. After another while autohealing recovers the faulty VM and all requests should return `This is server 1` again.
...
| <a name="input_secondary_ip"></a> [secondary\_ip](#input\_secondary\_ip) | IP address of the secondary VM instance | `string` | `"10.100.2.2"` | no |
