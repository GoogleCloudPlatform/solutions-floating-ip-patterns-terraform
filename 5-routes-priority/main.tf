...
resource "google_compute_instance_template" "nginx_primary_instance_template" {
  name_prefix  = "nginx-primary-template"
...
resource "google_compute_instance_template" "nginx_secondary_instance_template" {
  name_prefix  = "nginx-secondary-template"
  machine_type = local.machine_type
  disk {
    source_image = local.image
    auto_delete  = true
...
resource "google_compute_instance_group_manager" "nginx_primary_instance_group" {
  name               = "nginx-primary"
...
resource "google_compute_instance_group_manager" "nginx_secondary_instance_group" {
...
resource "google_compute_route" "primary_route" {
  depends_on = [google_compute_subnetwork.failover_subnet]

  name        = "floating-route-primary"
  dest_range  = "${var.floating_ip}/32"
  network     = google_compute_network.failover_vpc.name
  next_hop_ip = var.primary_ip
  priority    = 50
}

resource "google_compute_route" "secondary_route" {
  depends_on = [google_compute_subnetwork.failover_subnet]

  name        = "floating-route-backup"
  dest_range  = "${var.floating_ip}/32"
  network     = google_compute_network.failover_vpc.name
  next_hop_ip = var.secondary_ip
  priority    = 100
}
